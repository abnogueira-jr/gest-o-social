...
const EMPTY_FORM = { nome: "", descricao: "", bairros: "", ativo: true };

export default function TabelasRegioesCG() {
  ...
  const carregar = async () => {
    setLoading(true);
    const data = await base44.entities.RegiaoCG.list("nome", 200);
    setItens(data);
    setLoading(false);
  };
  ...
  const filtrados = itens.filter(i =>
    ...
    i.bairros?.toLowerCase().includes(busca.toLowerCase())
  );

  const abrirNovo = () => { setForm(EMPTY_FORM); setEditandoId(null); setModalAberto(true); };
  const abrirEditar = (item) => {
    setForm({ nome: item.nome || "", descricao: item.descricao || "", bairros: item.bairros || "", ativo: item.ativo !== false });
  ...
  const salvar = async () => {
    ...
    setSalvando(true);
    if (editandoId) {
      await base44.entities.RegiaoCG.update(editandoId, form);
      toast.success("Região atualizada.");
    } else {
      await base44.entities.RegiaoCG.create(form);
  ...
  const excluir = async (id) => {
    await base44.entities.RegiaoCG.delete(id);
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-800">Regiões de Campo Grande</h1>
      ...
      <div className="relative w-full sm:max-w-sm">
        ...
        <Input placeholder="Buscar por nome, descrição ou bairro..." className="pl-9" value={busca} onChange={(e) => setBusca(e.target.value)} />
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 hidden md:table-cell">Bairros</th>
            ...
            <tbody>
              {filtrados.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-3 py-3 text-slate-500 text-xs">{item.descricao || "—"}</td>
                  <td className="px-3 py-3 text-slate-400 text-xs max-w-xs truncate hidden md:table-cell">{item.bairros || "—"}</td>
      ...
      <Dialog open={modalAberto} onOpenChange={setModalAberto}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editandoId ? "Editar Região" : "Nova Região de Campo Grande"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label>Nome *</Label>
              <Input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} placeholder="Ex: Região Norte" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label>Bairros Abrangidos</Label>
              <Input value={form.bairros} onChange={(e) => setForm({ ...form, bairros: e.target.value })} placeholder="Ex: Bairro A, Bairro B, Bairro C" />
              <p className="text-[11px] text-slate-400">Separe os bairros por vírgula</p>
            </div>
            <div className="flex items-center gap-2">
